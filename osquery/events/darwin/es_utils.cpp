...
FLAG(string,
     es_enabled_events,
     "",
     "Comma-delimited list of specific EndpointSecurity event types to enable (e.g., 'notify_mount,notify_unmount,notify_setuid')");
...
std::map<std::string, es_event_type_t> kESEventNameMap = {
  ...
  {"notify_xp_malware_remediated", ES_EVENT_TYPE_NOTIFY_XP_MALWARE_REMEDIATED},
  ...
  {"notify_authorization_petition", ES_EVENT_TYPE_NOTIFY_AUTHORIZATION_PETITION},
  {"notify_authorization_judgement", ES_EVENT_TYPE_NOTIFY_AUTHORIZATION_JUDGEMENT},
  ...
  if (!FLAGS_es_enabled_events.empty()) {
    ...
    for (const auto& name : event_names) {
      ...
      if (it != kESEventNameMap.end()) {
        ...
        if (std::find(events.begin(), events.end(), it->second) == events.end()) {
